...
import 'package:products_app/ui/molecules/modals/app_loader.dart';
import 'package:products_app/utils/color_utils.dart';
import 'package:products_app/utils/extension/base_page_extension.dart';
    ...
    model.error.listen((event) {
      if (mounted) {
        if (event.error.message.isNotEmpty) {
          ...
          // ));
          // showTopError(errorsMessage[event.error.message]);
        }
      }
    });
  ...
  Future<bool> _onWillPop(dynamic param) {
    if (Navigator.canPop(context)) {
      if (param != null) {
        ...
        if (context.widget.toString() == "WalkthroughMainPage" ||
            context.widget.toString() == "LoginViewPage") {
          return Future.value(false);
        } else {
          Navigator.pop(
            context,
